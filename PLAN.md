...
## 7.4.1 `/parent/mission` ミッション設定

目的: 親が子どもごとの身近なミッションを設定・変更する。
...
- ミッションは子どもごとに現在1つ
- 保存すると前のミッションを上書きし、達成状態はクリアする
- 達成記録は `/parent/record` で親が行う
- 達成時は `transactions` に `type = grant` の取引を追加する
- 期限は親画面だけの判断材料とし、期限後でも親判断で達成記録できる
...
- current missionは子どもごとに1つ
- PoCでは `documents` のJSONとして保存してよい
- 保存時は同じ子の現在ミッションを上書きする
- 達成時は正の `grant` 取引を作り、`completed_transaction_id` で参照する
- 同じcurrent missionの二重達成を防ぐ
...
- 親が `/parent/record` で現在のミッション達成を記録する
- `transactions` に正の `grant` 取引を追加する
- labelは `ミッション: {ミッション名}` とする
- ミッション側に `completed_at` と `completed_transaction_id` を保存する
- 期限を過ぎていても親判断で達成記録できる
- 同じ現在のミッションは二重に達成記録できない
- 誤入力は通常の履歴取り消しで補正する
...
- ミッションは「みっしょん」と表示し、名前・報酬・達成済みの短い印だけにする
- ミッション期限切れや未達成の圧は子ども画面に出さない
- 子ども画面では買えるもの一覧を出さず、目標とタグ量に集中させる
- 目標だけは買えなくても常に表示する
- 目標画像は残し、何を目指しているかを絵で分かるようにする
- 目標画像は将来ユーザーが用意した画像を表示できる領域として広めに確保し、画像URLがなければプリセット絵を表示する
...
- children / transactions / goals / missions / shop_items / settings を実装する
...
- SQLite + Node APIへ移行する
- DBスキーマを作成する
- localStorageを本番データのsource of truthにしない
...
- Cloudflare Tunnel経由でVPS上のアプリへアクセスする
- Cloudflare Access JWTをアプリサーバー側で検証する
- accountsテーブルで `parent` / `child` ロールを管理する
- 子アカウントは `/kids` 表示のみ
- 親アカウントは `/parent/*` 編集可能
- 書き込みAPIは親ロールのみ許可する
...
- child roleで書き込みAPIが403になる
- parent roleで記録・取り消し・目標更新ができる
- unknown accountが拒否される
...
- 物理タグとアプリ残高のズレを説明可能
- 親以外が管理画面を操作できない
- VPS/Cloudflare Tunnel版では、子アカウントは表示のみ、親アカウントは編集可能にする
- 書き込みAPIはサーバー側で親ロールを検証する
...
- Cloudflare Accessを使う場合も、アプリサーバー側でAccess JWTとアカウントロールを検証する
...
- plan: Cloudflare Access + アプリ側ロール検証の境界を決める
- observe: localStorage依存と親操作API化が必要な箇所を確認する
- implement: Access JWT検証、accountsテーブル、parent-only API guardを実装する
- test: child roleで書き込みAPIが403になることを確認する
- refactor: 画面の表示ガードとAPI認可を分離する
- prune: 複数家庭対応やアプリ内ソーシャルログインを混ぜない
...
```json
[
  { "child": "あおい", "title": "といれにいく", "reward_amount": 1, "deadline_at": "2026-05-20T20:00:00+09:00" },
  { "child": "はる", "title": "じゅんびをする", "reward_amount": 1 }
]
```
